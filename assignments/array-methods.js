...
function runnerNames(arr) {
  let fullName = [];
  arr.forEach(function (runer) {
...
console.log(runnerNames(runners));
console.log(fullName);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result

function allCaps(arr, cb) {
  let allRunners = cb(arr);

  returnallRunners.map(function (runner) {
    runner.toUpperCase();
  });
}
...
function largeSizes(arr) {
  let largeShirts = arr.filter(function (runner) {
...
console.log(largeSizes(runners));
...
function calculatePrice(arr) {
  let donations = arr.map(function (runner) {
    return runner["donation"];
  });

  let ticketPriceTotal = arr.reduce(function (accumulator, currentValue) {
